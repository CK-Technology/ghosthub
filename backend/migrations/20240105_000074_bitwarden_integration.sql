...
-- Enables bi-directional sync between GhostHub password manager and external password managers
...
CREATE TABLE bitwarden_servers (
    ...
    -- Conflict Resolution
    conflict_resolution VARCHAR(30) DEFAULT 'manual', -- manual, bitwarden_wins, ghosthub_wins, newer_wins
    auto_resolve_conflicts BOOLEAN DEFAULT false,
    
    -- Mapping Rules
    folder_mapping JSONB DEFAULT '{}', -- Maps Bitwarden folders to GhostHub folders
...
CREATE TABLE bitwarden_sync_mappings (
    ...
    ghosthub_password_id UUID NOT NULL REFERENCES passwords(id) ON DELETE CASCADE,
    ...
    ghosthub_version INTEGER DEFAULT 1,
...
CREATE TABLE bitwarden_collections (
    ...
    -- Sync Settings
    sync_enabled BOOLEAN DEFAULT true,
    ghosthub_folder_id UUID REFERENCES password_folders(id) ON DELETE SET NULL,
...
CREATE TABLE bitwarden_sync_conflicts (
    ...
    -- Conflicting Data
    ghosthub_data JSONB NOT NULL,
    bitwarden_data JSONB NOT NULL,
    
    -- Timestamps
    ghosthub_modified_at TIMESTAMPTZ,
    ...
    resolution_choice VARCHAR(30), -- use_ghosthub, use_bitwarden, use_merged, use_custom
    resolved_data JSONB,
    resolved_by UUID REFERENCES users(id),
    resolved_at TIMESTAMPTZ,
...
CREATE INDEX idx_bitwarden_sync_mappings_password ON bitwarden_sync_mappings(ghosthub_password_id);
...
BEGIN
    ...
    WHERE bsm.ghosthub_password_id = NEW.id
    ...
    IF FOUND THEN
        ...
        IF OLD.updated_at IS DISTINCT FROM NEW.updated_at THEN
            ...
            SET sync_status = 'conflict',
                ghosthub_version = ghosthub_version + 1
            WHERE ghosthub_password_id = NEW.id;
            ...
            INSERT INTO bitwarden_sync_history (
                ...
                NEW.id,
                'warning',
                'GhostHub password modified - potential sync conflict',
                NOW(),
                NOW()
            );
        END IF;
...
BEGIN
    ...
    LOOP
        ...
        IF conflict_record.ghosthub_modified_at > conflict_record.bitwarden_modified_at THEN
            UPDATE bitwarden_sync_conflicts
            SET resolution_status = 'resolved',
                resolution_choice = 'use_ghosthub',
                resolved_data = conflict_record.ghosthub_data,
                resolved_at = NOW(),
                auto_resolution_attempted = true,
                auto_resolution_result = 'success',
                auto_resolution_reason = 'GhostHub data is newer'
            WHERE id = conflict_record.id;
            
            resolved_count := resolved_count + 1;
        ELSIF conflict_record.bitwarden_modified_at > conflict_record.ghosthub_modified_at THEN
            UPDATE bitwarden_sync_conflicts
            SET resolution_status = 'resolved',
...
COMMENT ON TABLE bitwarden_sync_mappings IS 'Maps GhostHub passwords to Bitwarden items for synchronization';

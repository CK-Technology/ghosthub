...
) -> Result<Json<Vec<ghosthub_shared::Client>>, StatusCode> {
    let limit = params.limit.unwrap_or(50);
    let offset = params.offset.unwrap_or(0);

    let query = if let Some(search) = params.search {
        sqlx::query_as!(
            ghosthub_shared::Client,
...
) -> Result<(StatusCode, Json<ghosthub_shared::Client>), StatusCode> {
...
async fn get_client(
    State(state): State<Arc<AppState>>,
    Path(id): Path<Uuid>,
) -> Result<Json<ghosthub_shared::Client>, StatusCode> {
    match sqlx::query_as!(
        ghosthub_shared::Client,
        "SELECT id, name, email, phone, address, city, state, zip, billing_address, notes, 
         created_at, updated_at, archived_at 
...
async fn get_client_contacts(
    State(state): State<Arc<AppState>>,
    Path(id): Path<Uuid>,
) -> Result<Json<Vec<ghosthub_shared::Contact>>, StatusCode> {
    match sqlx::query_as!(
        ghosthub_shared::Contact,
...
async fn get_client_assets(
    State(state): State<Arc<AppState>>,
    Path(id): Path<Uuid>,
) -> Result<Json<Vec<ghosthub_shared::Asset>>, StatusCode> {
    match sqlx::query_as!(
        ghosthub_shared::Asset,
...
async fn get_client_tickets(
    State(state): State<Arc<AppState>>,
    Path(id): Path<Uuid>,
) -> Result<Json<Vec<ghosthub_shared::Ticket>>, StatusCode> {
    match sqlx::query_as!(
        ghosthub_shared::Ticket,
